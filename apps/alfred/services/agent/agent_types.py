...
KNOWLEDGE_AGENT = AgentType(
    ...
    system_prompt=(
        ...
        "2. Use search_kb metadata arguments when the task mentions tags, labels, topics, "
        "or domains. Before reporting no match, retry plausible variants: lowercase, "
        "spaceless, hyphenated, camel-case split, acronym/internal-name variants, and "
        "metadata-only tag/topic search. Run independent search variants in the same turn "
        "so they execute concurrently.\n"
        "3. When a candidate looks relevant, call get_zettel or get_document to read the full body "
        "before citing it.\n"
        "4. For writes, prefer update_zettel on an existing card over creating a duplicate. "
