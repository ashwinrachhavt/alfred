...
class ResearchService:
    ...
    def _read_latest_from_db(self, topic: str) -> dict[str, Any] | None:
        ...
        *,
        ...
        with SessionLocal() as session:
            ...
            try:
                ...
                session.rollback()
                row = session.exec(
                    select(ResearchReportRow).where(
                        ResearchReportRow.topic_key == key
                    )
                ).first()
                if row is None:
                    raise
            ...
            stored.setdefault("topic", row.topic)
        ...
        if self._primary_search is None:
            if not (settings.searxng_host or settings.searx_host):
                raise ConfigurationError(
                    "SearxNG is required for research. Set SEARXNG_HOST (or SEARX_HOST)."
                )
    ...
    # Public API
    # ------------------------------------------------------------------
    def get_cached_report(self, topic: str) -> dict[str, Any] | None:
        topic = (topic or "").strip()
        if not topic:
            return None
        try:
            cached = self._read_latest_from_db(topic)
            if cached:
                return cached
        except Exception:
            logger.debug("Research DB cache lookup failed", exc_info=True)

        # Back-compat: older deployments may have written to the JSON document store.
        legacy = self._find_latest(topic)
        if legacy is None:
            return None
        try:
    ...
    def generate_report(self, topic: str, *, refresh: bool = False) -> dict[str, Any]:
        topic = topic.strip()
        if not topic:
            raise ValueError("Topic is required")

        if not refresh:
            cached = self.get_cached_report(topic)
            if cached:
                return cached

        sources, search_meta = self._collect_sources(topic)
        self._ensure_llm()
        report = self._run_llm(topic, sources)
        ...
        try:
            ...
            logger.exception("Failed to persist research report")
            # Fallback to the legacy JSON store so the feature still works even if the
            # relational table hasn't been migrated yet.
            self._get_store().update_one({"topic": topic}, {"$set": payload}, upsert=True)
            stored = self._find_latest(topic)
    ...
    def _collect_sources(self, topic: str) -> tuple[list[EnrichedSource], dict[str, Any]]:
        primary = self._search(topic, self._get_primary_search())
    ...
    def _run_llm(self, topic: str, sources: list[EnrichedSource]) -> ResearchReport:
        context = self._render_context(sources)
        user_prompt = REPORT_PROMPT.format(company=topic, sources=context)
        return cast(
            ResearchReport,
    ...
    def _find_latest(self, topic: str) -> dict[str, Any] | None:
        record = self._get_store().find_one({"topic": topic})
        if not record:
            return None
        payload = dict(record)
