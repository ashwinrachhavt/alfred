...
class InterviewQuestionsService:
    ...
    def __post_init__(self) -> None:
        if self.primary_search is None:
            ...
            self.primary_search = WebConnector(searx_k=self.search_results)
        ...
        *,
        ...
        if use_firecrawl_search and self.firecrawl is not None:
            for q in queries:
                ...
                for rank, item in enumerate(data[: max(1, int(self.firecrawl_search_results))]):
                    ...
                    extra_questions = self._extract_questions_from_text(snippet, max_questions=4)
                    _upsert_source(
                        url=str(url),
