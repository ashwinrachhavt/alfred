...
class PanelInterviewServiceProtocol(Protocol):
    def create_session(self, payload: PanelSessionCreate) -> Any: ...

    def submit_turn(self, session_id: str, payload: PanelTurnRequest) -> Any: ...
...
def _coerce_optional_uuid(value: str | uuid.UUID | None) -> uuid.UUID | None:
    """Best-effort parse for optional UUID identifiers.

    Treats empty/whitespace values and common JS-y sentinels (e.g. "undefined")
    as missing.
    """
...
def _normalize_question_item(item: Any) -> dict[str, Any]:
    return {
        "question": _safe_text(getattr(item, "question", None) or item.get("question")),
        "categories": list(getattr(item, "categories", None) or item.get("categories") or []),
        "occurrences": int(getattr(item, "occurrences", None) or item.get("occurrences") or 1),
        "sources": list(getattr(item, "sources", None) or item.get("sources") or []),
    }
...
class UnifiedInterviewAgent:
    ...
    async def _collect_questions_node(self, state: InterviewAgentState) -> dict[str, Any]:
        try:
            ...
            raw = [_normalize_question_item(item) for item in getattr(report, "questions", [])]
            sources = getattr(report, "sources", []) or []
            return {"raw_questions": raw, "sources_scraped": len(sources)}
        except Exception as exc:  # pragma: no cover
        ...
        try:
            ...
            if state.get("candidate_response"):
                resp = self.panel_service.submit_turn(
                    str(session_id),
                    PanelTurnRequest(answer=_safe_text(state.get("candidate_response"))),
                )
                resp_events = getattr(resp, "events", None) or resp.get("events") or []
                events = [
                    e.model_dump(mode="json") if hasattr(e, "model_dump") else dict(e)
                    for e in resp_events
                ]

            return {"session_id": str(session_id), "practice_events": events}
        except Exception as exc:  # pragma: no cover
...
def agent():
    ...
    This exposes the compiled internal graph; the public API is `UnifiedInterviewAgent.process`.
    """
