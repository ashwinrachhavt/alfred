...
import { DecomposePanel } from "./decompose-panel";
...
function SessionSidebar({ activeId }: { activeId: number }) {
 ...
 <h2 className="text-lg tracking-tight">Think</h2>
 <Button
 size="sm"
 variant="ghost"
 onClick={handleNew}
...
export function SessionEditorShell({ sessionId }: { sessionId: number }) {
 ...
 <SessionEditor
 sessionId={sessionId}
 onToggleDecompose={() => setShowDecompose((prev) => !prev)}
 showDecompose={showDecompose}
 />
 </div>

 {/* Right panel: decompose (collapsible) */}
 {showDecompose ? (
 <div className="w-80 shrink-0 overflow-hidden border-l">
 <DecomposePanel onClose={() => setShowDecompose(false)} />
 </div>
 ) : null}
 </div>
 );
