...
export function ReviewQuiz({ zettels }: Props) {
  ...
  if (withQuiz.length === 0) {
    return (
      <div className="py-16 text-center">
        <p className="font-serif text-xl text-muted-foreground">No quiz questions available</p>
      </div>
    );
  }
  ...
  if (index >= withQuiz.length || !zettel || !q) {
    return (
      <div className="mx-auto max-w-md py-12 text-center">
        ...
        <Button className="mt-6 font-mono text-xs" onClick={() => { setIndex(0); setSelected(null); setScore({ correct: 0, total: 0 }); }}>
          Retake Quiz
        </Button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-lg py-8">
      ...
      <p className="font-serif text-xl leading-relaxed mb-6">{q.question}</p>
      ...
      <div className="space-y-2">
        {options.map((opt, i) => {
          ...
          return (
            ...
            >
              <span
                className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full border font-mono text-[10px]"
      ...
      {answered && (
        <div className="mt-4 flex items-center justify-between">
          <span className={cn("font-mono text-xs", isCorrect ? "text-[var(--success)]" : "text-[var(--destructive)]")}>
            {isCorrect ? "Correct!" : "Incorrect"}
          </span>
          <Button className="font-mono text-xs" onClick={handleNext}>
