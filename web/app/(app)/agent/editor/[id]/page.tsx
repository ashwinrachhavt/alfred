...
export default function ZettelEditorPage() {
 ...
 className="flex-1 bg-transparent text-lg text-[var(--text-primary)] border-none outline-none"
 />
 </div>
 ...
 {zettel.topic && (
 <span className="text-[10px] uppercase tracking-[0.1em] text-[var(--accent)] px-1.5 py-0.5 rounded bg-[var(--accent-subtle)]">
 {zettel.topic}
 </span>
 )}
 {zettel.tags?.map((tag) => (
 <span key={tag} className="text-[10px] text-[var(--text-tertiary)] px-1.5 py-0.5 rounded bg-[var(--bg-tertiary)]">
 {tag}
 </span>
 ))}
 </div>
