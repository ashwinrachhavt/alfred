      ...
      <div className="flex items-center gap-2 px-6 py-2 border-b">
        ...
        <div className="ml-auto flex items-center gap-1">
          <Button
            variant="ghost"
            size="sm"
            className="h-7 gap-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"
        ...
        <div className="max-w-2xl mx-auto px-6 py-4">
            ...
            <div className="flex items-center gap-2 mb-3">
              {PHILOSOPHICAL_LENSES.map((l) => (
                <button
                  ...
                  className={cn(
                    "rounded-full px-3 py-1 text-[11px] font-mono transition-colors border",
                    activeLens === l.id
          ...
          <div className="flex items-end gap-0 rounded-xl border bg-card shadow-sm focus-within:border-primary transition-colors">
            ...
            <div className="flex items-center gap-1 px-2 py-2">
              ...
              {/* Model indicator */}
              <button className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-mono text-muted-foreground hover:text-foreground transition-colors">
                <span className="text-primary">★</span>
                <select
                  value={activeModel}
                  onChange={(e) => setModel(e.target.value)}
                  className="bg-transparent outline-none cursor-pointer text-[11px] font-mono"
                >
      ...
      {message.gaps.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {message.gaps.map((gap) => (
            <span
              key={gap.concept}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-mono bg-[var(--alfred-accent-subtle)] text-primary"
            >
              gap: {gap.concept}
            </span>
          ))}
