...
- Bible: `web/docs/design-system-bible.md`
...
The frontend expects the Alfred FastAPI server to be running. During local development,
the Next.js dev server proxies all `/api/*` requests to `http://localhost:8000` by default.

If your API runs elsewhere, set:

```bash
export ALFRED_API_BASE_URL="http://localhost:8000"
```
