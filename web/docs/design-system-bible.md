...
- **Tokens & base styles**: `web/app/globals.css`
- **UI primitives (shadcn-style, Radix-based)**: `web/components/ui/*`
- **Feature UI**: `web/features/*`
- **Reusable layout primitives**: `web/components/layout/*`
- **Classname utility**: `web/lib/utils.ts` (`cn`)
- **In-app showcase**: `web/app/(app)/design-system/page.tsx`
...
- Light: `:root` in `web/app/globals.css`
- Dark: `.dark` in `web/app/globals.css`
...
- Tailwind v4 maps semantic variables to utilities via `@theme inline` (see the `--color-*` declarations in `web/app/globals.css`).
- Prefer utilities like `bg-background`, `text-foreground`, `border-border` rather than raw color values.

Adding/adjusting a token:

1. Update `:root` and `.dark` values in `web/app/globals.css` (use **OKLCH** like the rest of the file).
...
- Font families are configured in `web/app/layout.tsx` via `next/font` (Geist + Geist Mono).
- Tailwind uses `font-sans` / `font-mono` mapped in `web/app/globals.css` (`--font-sans`, `--font-mono`).
...
- Use `Page` from `web/components/layout/page` for consistent page paddings and widths.

### Radius

Radius is tokenized:

- Base token: `--radius` in `web/app/globals.css`
...
- Prefer existing primitives in `web/components/ui/*` before creating a new component.
- Variants should use `class-variance-authority` (CVA), as in `web/components/ui/button.tsx`.
...
Use `Button` from `web/components/ui/button.tsx`.
...
- Sidebar primitives: `web/components/ui/sidebar.tsx`
...
- Empty states: `web/components/ui/empty-state.tsx`
...
- Use `Page` (`web/components/layout/page`) to enforce consistent widths and padding.
...
- Theme switching uses `next-themes` via `web/components/theme-provider.tsx`.
...
- Reuses primitives where possible (`web/components/ui/*`)
