...
Compose passes those files into Alfred and Firecrawl containers at runtime. The stack then overrides only container-network values such as `DATABASE_URL`, `REDIS_URL`, `QDRANT_LOCAL_URL`, `FIRECRAWL_BASE_URL`, and `SEARXNG_HOST`.
...
```bash
ALFRED_WEB_PORT=3020 ALFRED_API_PORT=8010 docker compose up --build
```
