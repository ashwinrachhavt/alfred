...
def test_list_contacts_filters_cache_by_provider(monkeypatch) -> None:
    session = _session()
    contact = OutreachContact(
        run_id=1,
        company="Stripe",
    ...
    def _fake(self, company: str, limit: int = 20, **kwargs):
        called["providers"] = kwargs.get("providers")
        return [{"email": "apollo@stripe.com", "title": "", "confidence": 0.8, "source": "apollo"}]

    monkeypatch.setattr(
        "alfred.services.company_outreach_service.ContactDiscoveryService.discover", _fake
    )

    # Provider matches cache -> no discovery call needed.
    results = svc.list_contacts(
        "Stripe", limit=5, role_filter=None, refresh=False, providers=[ContactProvider.HUNTER]
    )
    assert results[0]["email"] == "jane@stripe.com"
    assert called == {}

    # Provider excludes cache -> falls through to discovery.
    results = svc.list_contacts(
        "Stripe", limit=5, role_filter=None, refresh=False, providers=[ContactProvider.APOLLO]
    )
    assert results[0]["email"] == "apollo@stripe.com"
